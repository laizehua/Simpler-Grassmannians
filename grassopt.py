...
class EAS_trace_opt:
    ...
    def EAS_linesearch_svd(self, Y, U, s, V):
        list = []
        for i in np.arange(20, dtype = float):
            list.append(self.EAS_function(self.EAS_geodesic_svd(Y, U, s*(2**(i-10)), V)))
        j = np.argmin(list).astype(float)
        return s*(2**(j-10)), self.EAS_geodesic_svd(Y, U, s*(2**(j-10)), V)
    ...
    def EAS_steepest_gradient(self, Y_0, t, orthogonalize = False):
        ...
        for i in range(t):
            ...
            j, Y = self.EAS_linesearch_svd(Y, U, s, V)
    ...
    def EAS_conjugate_gradient(self, Y_0, t, reset, orthogonalize = False):
        ...
        for i in range(t):
            ...
            s, Y_new = self.EAS_linesearch_svd(Y_old, U, s, V)
            Ylist.append(Y_new)
            if orthogonalize == True:
...
class Simpler_trace_opt:
    ...
    def simpler_backtracking_svd(self, Q, U, s, V):
    
        c=1
        fun1 = self.simpler_function(Q)
        for i in range(5,-20,-1):
            Q2 = self.simpler_geodesic_svd(Q, U, 2**(i)*s, V)
            fun2 = self.simpler_function(Q2)
            if fun1-fun2>c*(2**(i))*(2*linalg.norm(s)**2):
                return 2**(i)*s, Q2
        print("failed!")
        return s, self.simpler_geodesic_svd(Q, U, s, V)

    def simpler_linesearch_svd(self, Q, U, s, V):
        
        list = []
        for i in np.arange(20, dtype = float):
            Q_new = self.simpler_geodesic_svd(Q, U, s*(2**(i-10)), V)
            list.append(self.simpler_function(Q_new))
        j = np.argmin(list).astype(float)
        return s*(2**(j-10)), self.simpler_geodesic_svd(Q, U, s*(2**(j-10)), V)

    def simpler_wolfe_linesearch_svd(self, Q, U, s, V):
        
        list = []
        S = U.dot(np.diag(s)).dot(V[:self._m,:])
        a = min(np.sum(np.multiply(self.simpler_gradient(Q),S)), 0)
        for i in np.arange(20, 0, -2, dtype = float):
            Q_new = self.simpler_geodesic_svd(Q, U, s*(2**(i-10)), V)
            list.append(self.simpler_function(Q_new))
            if ((self.simpler_function(Q_new)-self.simpler_function(Q)< a*0.01*(2**(i-10))) &
                (-np.sum(np.multiply(self.simpler_gradient(Q_new),S))<-0.9*a)):
    ...
    def simpler_steepest_gradient(self, Q_0, t):
        ...
        for i in range(t):
            ...
            U, s, V =  linalg.svd(-B)
            s, Q = self.simpler_linesearch_svd(Q, U, s, V)
        return Q
    
    def simpler_steepest_gradient2(self, Q_0, t):
        Q=Q_0
        for i in range(t):
            B = self.simpler_gradient(Q)
            U, s, V =  linalg.svd(-B)
            s, Q = self.simpler_backtracking_svd(Q, U, s, V)
        return Q
    ...
    def simpler_conjugate_gradient(self, Q_0, t, reset):
        ...
        for i in range(t):
            U, s, V =  linalg.svd(P)
            s, Q_new = self.simpler_linesearch_svd(Q_old, U, s, V)
            Qlist.append(Q_new)
...
class EAS_Frechet_opt:
    ...
    def EAS_backtracking_svd(self, Y, U, s, V):
    
        c=0.5
        fun1 = self.EAS_function(Y)
        for i in range(50):
            Y2 = self.EAS_geodesic_svd(Y, U, 2**(10-i)*s, V)
            fun2 = self.EAS_function(Y2)
            if fun1-fun2>c*2**(10-i):
                return 2**(10-i)*s, Y2
        return s, self.EAS_geodesic_svd(Y, U, s, V)
...
class Simpler_Frechet_opt:
    ...
    def simpler_linesearch_svd(self, Q, U, s, V, steplength = 2):
        
        list = []
        c = steplength
        for i in np.arange(30, dtype = float):
            Q_new = self.simpler_geodesic_svd(Q, U, s*(c**(i-20)), V)
            list.append(self.simpler_function(Q_new))
        j = np.argmin(list).astype(float)
        return s*(c**(j-20)), self.simpler_geodesic_svd(Q, U, s*(c**(j-20)), V)
    ...
    def simpler_steepest_gradient(self, Q_0, t, steplength = 2):
        ...
        for i in range(t):
            B = self.simpler_gradient(Q)
            U, s, V =  linalg.svd(-B)
            s, Q = self.simpler_linesearch_svd(Q, U, s, V, steplength = steplength)
        return Q
